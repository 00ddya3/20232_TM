...
from nurphy.bert.bert_dataset import PYBERTDataset
from nurphy.bert.bert_classification_model import PYBERTClassifier
from nurphy.bert.bert_predictor import bert_predictor
from nurphy.bert.bert_trainer import PYBERTTrainer
from nurphy.bert.bert_util import create_data_loader, add_sentiment_label, convert_to_df, get_korean_tokenizer, show_confusion_matrix
from transformers import BertModel, BertTokenizer
from sklearn.model_selection import train_test_split
from nurphy.bert.tokenization_kobert import KoBertTokenizer
...
if mode == 'en':
    ...
    pipeline = ptm.Pipeline(ptm.splitter.NLTK(),
                            ...
                            ptm.lemmatizer.SejongPOSLemmatizer(),   #원형복원
                            ptm.helper.SelectWordOnly(),
                            ptm.helper.StopwordFilter(file=stopwords))

    corpus = ptm.CorpusFromFieldDelimitedFileForClassification(input_file, delimiter='\t', doc_index=1, class_index=2)  #파일, 구분자, 문서 인덱스, 분류 인덱스
    ...
    #여기서부터
    for idx, item in enumerate(corpus.docs):
        if idx == 0:
            continue
        cla_label = corpus.pair_map[idx]
        documents.append(item)
        labels.append(cla_label)

    '''
    for item in corpus.pair_map:
        documents.append(corpus.docs[item])
        labels.append(corpus.pair_map[item])
    #여기까지
    '''
...
# dataframe 무조건 만들어야 함 (인풋이 df 형태이기 때문)
print(df.head())
print(df.info())
...
tokenizer = get_korean_tokenizer(bert_model_name)   # monologue tokenizer
