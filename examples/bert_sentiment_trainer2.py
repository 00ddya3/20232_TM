...
from nurphy.bert.bert_classification_model import PYBERTClassifier, PYBERTClassifierGenAtten, PYBertForSequenceClassification
from nurphy.bert.bert_trainer import PYBERTTrainer
from nurphy.bert.bert_util import create_data_loader, add_sentiment_label, convert_to_df, get_korean_tokenizer
...
if mode == 'en':
    ...
    for idx, item in enumerate(corpus.pair_map):
        if idx == 0:
            continue
        documents.append(corpus.docs[item])
        if len(corpus.pair_map[item].strip()) > 0:
...
df_val, df_test1 = train_test_split(df_test, test_size=0.5, random_state=RANDOM_SEED)
